...
void st_propagate_node(t_st_node *a, t_st_node b) {
  st_node_assign(a, a->min + b.min, a->max + b.max);
}

void st_propagate(int si) {
  st_propagate_node(&st_lazy[2 * si + 1], st_lazy[si]);
  st_propagate_node(&st_lazy[2 * si + 2], st_lazy[si]);
  st_propagate_node(&st[2 * si + 1], st_lazy[si]);
  st_propagate_node(&st[2 * si + 2], st_lazy[si]);
  st_node_assign(&st_lazy[si], 0, 0);
}

void st_update(int si, int sl, int sr, int ul, int ur, int x) {
  if (sl == ul && sr == ur) {
    st_operate_const(&st_lazy[si], x);
  ...
  int m = (sr + sl) / 2;
  if (ur <= m)
    st_update(2 * si + 1, sl, m, ul, ur, x);
  else if (ul >= m + 1)
    st_update(2 * si + 2, m + 1, ur, ul, ur, x);
  else {
    st_update(2 * si + 1, ul, m, ul, ur, x);
    st_update(2 * si + 2, m + 1, ur, ul, ur, x);
  }
...
int main() {
  ...
  while (cin >> n) {
    ...
    for (i = 0; i < q; i++) {
      ...
      switch (k) {
        case 1:
          st_update(0, 0, n - 1, j - 1, j - 1, l - arr[j - 1]);
          break;
        case 2:
